...
for status, avg in avg_by_default.items():
    print(f"  {'Bad (1)' if status == 1 else 'Good (0)'}: {avg:.2f}")

fig, axes = plt.subplots(1, 3, figsize=(18, 5))
sns.histplot(df[col], kde=True, ax=axes[0], color='lightgreen')
sns.boxplot(y=df[col], ax=axes[1], color='gold')
sns.barplot(data=df, x='default', y=col, ax=axes[2], palette='Pastel1')
