...
plt.hist(not_default, bins=20, alpha=0.5, color='darkgreen', label='No Default (0)', density=True)
plt.hist(default, bins=20, alpha=0.5, color='moccasin', label='Default (1)', density=True)
sns.kdeplot(not_default, color='darkgreen', linewidth=2, label='No Default (0)')
sns.kdeplot(default, color='moccasin', linewidth=2, label='Default (1)')
plt.title(f'Split Density Plot of {col} by Loan Status', fontsize=16, pad=20)
plt.xlabel('Other Debts (in $1,000s)')
plt.ylabel('Density')
plt.legend()
